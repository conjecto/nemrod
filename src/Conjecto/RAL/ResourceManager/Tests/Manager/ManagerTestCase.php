...
class ManagerTestCase extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->repoFactory = $this->getMockBuilder('Conjecto\RAL\ResourceManager\Manager\RepositoryFactory')->setConstructorArgs(array('foo'));
    ...
    protected function getMockedResource($className, $uri, $graph)
    {
        $mockedResource = $this/*->getMockBuilder('Resource')
            ->setMethods(array('setRm', 'getUri', 'getGraph'))*/->getMock('Conjecto\RAL\ResourceManager\Resource\Resource', array('setRm', 'getUri', 'getGraph'));
    ...
    {
        return $this->getMockBuilder('Conjecto\RAL\ResourceManager\Manager\UnitOfWork')->setConstructorArgs(array($this->manager, 'http://foo.fr'))->getMock();
    }
